...
Setting up the GuestShell with guest-ip enabled and configured with DNS is done via the following from the routers command line, note this is done from Privileged EXEC mode and not global configuration mode. Here we shall use the Guestshell enable command, create a virtual port group (VPG) and added a DNS name-server. 

```
device-xe#guestshell enable virtualPortGroup 0 guest-ip 192.168.35.2 name-server 208.67.222.222
```

Next we can install “GIT” within GuestShell (if you enabled this in the Linux at the egde lab skip this and confirm git is installed with git version command)
...
Then, clone the code repo for the lab
