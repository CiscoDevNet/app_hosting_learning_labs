...
At the enable prompt run `guestshell run python` command and test the Python interpreter.
...
Now lets run some Python. The Python interactive mode allows us to execute Python functions from the Cisco Python CLI module to configure the device. Here will have Python issue a 'show route' command.
